    ...
    public partial class TTP : Form
    {
        ...
        public void CreateNewTab(string Title)
        {
            if (!string.IsNullOrEmpty(Title))
            {
                ...
                //panel child of panel1 that holds Groups
        ...
        private void MS_workspace_ItemClicked(object? sender, ToolStripItemClickedEventArgs e)
        {
            ...
            if(e.ClickedItem.Text == GTMS_DeleteGroup_Name)
            {
                ...
                DeleteGroup(DialogPrompts.DeleteGroupDialog(GT_DeleteForm_Text, GT_DeleteFrom_Prompt, groupItems), groupItems);
            }
        }
        ...
        public void CreateNewGroup(string title, GrpPanel grpPanel)
        {
            if (!string.IsNullOrEmpty(title))
            {
                ...
                {
                    if (item.GroupTitle == TitleFormated)
                        return;
                }

                SplitContainer splitContainer = (SplitContainer)grpPanel.Parent.Parent;

                Panel catpanel = new Panel();
                catpanel.Dock = DockStyle.Top;

                GroupItem groupItem = new GroupItem(TitleFormated);
                groupItem.Text = TitleFormated;
                groupItem.Dock = DockStyle.Fill;
                groupItem.TaskPanelHolder = CreateNewTaskPanel(groupItem.Text, splitContainer);
                groupItem.Click += Button_Click;

                grpPanel.groupItems.Add(groupItem);

                catpanel.Controls.Add(groupItem);
                grpPanel.Controls.Add(catpanel);
            }
        }

        public void DeleteGroup(List<string> selectedGroupsNames, List<GroupItem> GroupList)
        {
